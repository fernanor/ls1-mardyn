...
void MoleculeStorage::uploadState() {
	...
	for( int i = 0 ; i < _numCells ; i++ ) {
		...
		for( std::list<Molecule*>::const_iterator iterator = particles.begin() ; iterator != particles.end() ; iterator++ ) {
			...
			positions.push_back( *(float3*) &molecule.position );
			currentIndex++;
		}
	}
	...
	_startIndexBuffer.copyElementsToDevice( startIndices );
	_positionBuffer.copyElementsToDevice( positions );

	_forceBuffer.resize( sizeof( float3 ) * positions.size() );
...
void MoleculeStorage::downloadResults() {
	...
	_forceBuffer.copyElementsToHost( forces );
